...
foreach (string inputFile in Ioh.inputFiles){
    Ioh.inputFile = new StreamReader(inputFile);
    while (true) {
...
class LineHandler {    
    ...
    public void appendNewWord(string wordToAppend) {
        if (paragraphEnderChars.Contains(wordToAppend.Last()) && fitsOnLine(lineWords, wordToAppend, symbolsPerLine)) { // that means we at the end of the paragraph, so just print it without its paragraph breaker but tell iohandler to break it
            lineWords.Add(wordToAppend.TrimEnd(wordToAppend.Last()));
            // write the line using iohandler
            var text = lineWords;
            int normalSpaces = 1;
            int lastSpace = 1;
            iohandler.writeFormattedOutput(text, normalSpaces, lastSpace, breakParagraph:true);
...
class InputOutputHandler{
    ...
    public InputOutputHandler(string[] input, char[] paragraphEnderChars){
        ...
        try {
            ...
            if (input.Last() == "h"){
                ...
                inputFiles = input.Take(input.Count() - 3).ToList(); // turn this to an array now and later just loop over it creating new stream readers
                outputFile = new StreamWriter(input[input.Count() - 3]);
                symbolsPerLine = Convert.ToInt32(input[input.Count() - 2]);
            }
    ...
    public void writeFormattedOutput(List<string> wordsToWrite, int sizeOfMostSpaces, int sizeOfLastSpace, bool breakParagraph = false){
        ...
        string lineEnd = highlightMode ? "<-\n" : "\n";
        writeWordCharByChar(lineEnd);
        if (breakParagraph) { writeWordCharByChar(lineEnd); }
    }
